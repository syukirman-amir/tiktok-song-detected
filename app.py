...
def get_tiktok_path(url):
    ...
    if path.startswith('/'):
        path = path[1:]  # Removing the '/' character at the beginning of the path
...
def main():
    ...
    full_url = st.text_input("Example: https://www.tiktok.com/@username/video/1234567890")
    ...
    if st.button("Detected Music"):
        ...
        if path:
            ...
            if play_url:
               # st.success(f"Desired URL: {play_url}")
                #st.audio(play_url)  # Displaying the audio using Streamlit's audio widget
